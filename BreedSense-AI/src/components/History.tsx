...
import { loadIdentifications, removeIdentification } from '../utils/storage';
...
export function History() {
  ...
  const [persisted, setPersisted] = useState(() => loadIdentifications());
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            <span className='text-gray-400'>Activity History</span>
          </CardTitle>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
            ...
            <Card>
              <CardContent className="pt-4 text-center">
                ...
                <p className="text-xs text-gray-400">Identifications</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-4 text-center">
                <div className="text-2xl font-bold text-green-600">{stats.videos}</div>
                <p className="text-xs text-gray-400">Video Analyses</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-4 text-center">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="pl-10 bg-black text-white border-green-500/30 placeholder:text-gray-500 hover:border-green-500/50 focus:border-green-500/70"
              />
            </div>

            <Select value={typeFilter} onValueChange={setTypeFilter}>
              <SelectTrigger className="w-full md:w-48 bg-black text-white border-green-500/30 placeholder:text-gray-500 hover:border-green-500/50 focus:border-green-500/70">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className="w-full bg-black border-green-500/30 text-white md:w-48">
            ...
            <Button
              variant="outline"
              className="w-full md:w-auto bg-black border-green-500/30 text-white"
              onClick={() => {
                setSearchTerm('');
                setTypeFilter('all');
            ...
            <Button variant="outline" className=" bg-black border-green-500/30 text-white" onClick={exportHistory}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-2">
                              ...
                              <Button className='bg-red-700 border-red-700 hover:bg-red-800 hover:border-red-800 text-red-100'
                                  variant="destructive"
                                  size="sm"
                                  onClick={() => {
                                    const id = (record.data as any)?.id;
                                    if (!id) {
                                      toast.error('Unable to delete: missing id');
                                      return;
                                    }
                                    const ok = removeIdentification(id);
                                    if (ok) {
                                      setPersisted(prev => prev.filter(p => p.id !== id));
                  ...
                  <CardContent className="pt-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            <h3 className="font-medium text-white truncate">{record.title}</h3>
