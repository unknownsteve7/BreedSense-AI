      ...
      <Card className="bg-card/90 border-green-500/20">
        ...
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-2 gap-8 bg-muted/50">
              <TabsTrigger value="create" className="bg-black text-white border-green-400/20 data-[state=active]:bg-green-500/20 data-[state=active]:text-green-400">Create Report</TabsTrigger>
              <TabsTrigger value="history" className="bg-black text-white border-green-400/20 data-[state=active]:bg-green-500/20 data-[state=active]:text-green-400">Report History</TabsTrigger>
            </TabsList>
              ...
              <Card className="bg-card/50 border-green-500/20">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-sm font-medium mb-2 text-gray-400">District *</label>
                      <Select
                        value={reportData.district}
                        onValueChange={(value: string) => setReportData(prev => ({ ...prev, district: value }))}
                      >
                        <SelectTrigger
                          className='bg-black text-white border-green-500/30 placeholder:text-gray-500 hover:border-green-500/50 focus:border-green-500/70'>
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2 text-gray-400">Block *</label>
                      <Select
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2 text-gray-400">Village *</label>
                      <Input
                        className='bg-black text-white border-green-500/30 placeholder:text-white hover:border-green-500/50 focus:border-green-500/70'
                  ...
                  <div className="mt-4">
                    <label className="block text-sm font-medium mb-2 text-gray-400">Report Date</label>
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button variant="outline" className="w-full bg-black border-green-500/30 text-white md:w-auto">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {healthOptions.map((option) => (
                      <label key={option} className="flex items-center space-x-2 cursor-pointer">
                        <Checkbox className='border-green-500/30 text-green-500 bg-black hover:bg-green-500/10'
                  ...
                  <div className="space-y-3">
                    {persisted.map((identification) => (
                      <label key={`local-${identification.id}`} className="flex items-center space-x-3 p-3 border rounded-lg cursor-pointer">
                        <Checkbox
                          className='border-green-500/30 text-green-500 bg-black hover:bg-green-500/10'
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-400">
                            Confidence: {Math.round((identification.confidence || 0))}%
                          </p>
                        </div>
                      </label>
                    ))}
                  ...
                  <CardContent className="pt-6">
                    ...
                    {report.notes && (
                      <div>
                        ...
                        <p className="text-sm text-gray-400 mt-1">{report.notes}</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
