...
export function Camera({ onCapture, onFileUpload }: CameraProps) {
    ...
    try {
      ...
      if (videoRef.current) {
        ...
        videoRef.current.onerror = (e) => {
          ...
          setIsLoading(false);
          toast.error('Camera video error');
        };
  ...
  const captureAndAnalyze = useCallback(() => {
    ...
    const imageId = `captured_${Date.now()}`;
    localStorage.setItem(`camera_image_${imageId}`, imageData);
    ...
    const existingImages = JSON.parse(localStorage.getItem('camera_images') || '[]');
    existingImages.push(imageMetadata);
    localStorage.setItem('camera_images', JSON.stringify(existingImages));
    ...
    onCapture(imageData);
    // Removed toast message here as it's already handled in App.tsx
  }, [onCapture]);
  ...
  React.useEffect(() => {
    ...
    if (video) {
      const handleError = () => {
        ...
        toast.error('Camera feed error. Please try restarting the camera.');
        setIsStreaming(false);
      };
        ...
        <CardContent className="p-6">
          <div className="relative aspect-video bg-muted rounded-lg overflow-hidden mb-4">
            ...
            <video
              ...
              playsInline
              muted
              className={`w-full h-full object-cover cursor-crosshair ${isStreaming ? 'block' : 'hidden'}`}
            ...
            {/* Overlay content when streaming */}
            {isStreaming && (
              <>
                {focusPoint && (
                  <div
            ...
            {/* Loading/placeholder content when not streaming */}
            {!isStreaming && (
                ...
                <div className="text-center">
                  {isLoading ? (
                    <>
                      ...
                      <p className="text-sm mt-2 text-green-300">Please wait while we access your camera</p>
                    </>
                  ) : (
                    <>
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {!isStreaming ? (
              <Button
                onClick={startCamera}
                disabled={isLoading}
                className="min-w-32 bg-green-500 hover:bg-green-400 text-black disabled:opacity-50"
              >
                <CameraIcon className="w-4 h-4 mr-2" />
                {isLoading ? 'Starting...' : t('camera.startCamera')}
              </Button>
            ) : (
              <>
                <Button
                  ...
                  size="lg"
                  className="min-w-40 bg-green-500 hover:bg-green-400 text-black font-semibold"
                >
                  <CameraIcon className="w-4 h-4 mr-2" />
                  Capture & Analyze
                </Button>
