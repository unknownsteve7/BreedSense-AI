...
import { MapPin, Edit3, Check, Eye, Volume2, Download, AlertTriangle, Calendar, Scale, Droplets, Award, Heart, Info } from 'lucide-react';
import type { DetectionResult } from '../types';
// ...existing code... (no useEffect needed)
...
export function BreedIdentification({
  ...
  const [selectedBreed, setSelectedBreed] = useState<string>(results[0]?.breed.id || '');
  const [isEditing, setIsEditing] = useState(false);
  const [notes, setNotes] = useState('');
  const [showExplanation, setShowExplanation] = useState(false);
  const [breedOptions, setBreedOptions] = useState<Array<{ id: string; name: string }>>([]);

  // Defer loading full breed list until user opens the correction editor
  const loadBreedOptionsIfNeeded = async () => {
    if (breedOptions.length > 0) return;
    try {
      const names = await getBuffaloBreeds();
      setBreedOptions(names.map((n: string) => ({ id: n, name: n })));
    } catch (e) {
      console.warn('Failed to load breed names', e);
      toast.error('Unable to load breed list for correction');
    }
  };
  ...
  const handleCorrection = () => {
    if (selectedBreed && onCorrection) {
      onCorrection(selectedBreed);
      setIsEditing(false);
      toast.success('Breed correction saved successfully');
    }
  };
        ...
        <CardContent>
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
                  ...
                  <div key={index} className="p-4 border border-green-500/20 rounded-lg bg-green-500/5">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Button
                        ...
                        onClick={() => handleTTS(result.breed?.description ?? '')}
                      >
              ...
              {/* Correction Section */}
              <div className="p-4 bg-muted/20 border border-green-500/20 rounded-lg">
                <div className="flex items-center justify-between mb-3">
                  <span className="font-medium text-green-400">Correct the prediction?</span>
                  <Button
                    variant="outline"
                    size="sm"
                    className="border-green-500/30 hover:bg-green-500/10"
                    onClick={async () => {
                      ...
                      setIsEditing(next);
                      if (next) await loadBreedOptionsIfNeeded();
                    }}
                  >
                ...
                {isEditing ? (
                  <div className="space-y-3">
                    <Select value={selectedBreed} onValueChange={setSelectedBreed}>
                      <SelectTrigger className="bg-muted/50 border-green-500/30">
                    ...
                    <Textarea
                      ...
                      rows={2}
                      className="bg-muted/50 border-green-500/30 text-white placeholder:text-muted-foreground"
                    />
                  ...
                  <p className="text-sm text-muted-foreground">
                    Help improve AI accuracy by correcting incorrect predictions
                  </p>
                )}
              </div>
            </div>
          ...
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-green-400">
              ...
              <Button
                ...
                size="sm"
                className="hover:bg-green-500/20"
                onClick={() => handleTTS(topResult.breed?.description ?? '')}
              >
          ...
          <CardContent>
            <Tabs defaultValue="overview" className="w-full">
              <TabsList className="grid w-full grid-cols-5 bg-muted/50">
                ...
                <TabsTrigger value="gender" className="data-[state=active]:bg-green-500/20 data-[state=active]:text-green-400">Gender Traits</TabsTrigger>
                <TabsTrigger value="production" className="data-[state=active]:bg-green-500/20 data-[state=active]:text-green-400">Production</TabsTrigger>
                <TabsTrigger value="history" className="data-[state=active]:bg-green-500/20 data-[state=active]:text-green-400">History</TabsTrigger>
                  ...
                  <div className="flex-1">
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <Badge className={`${topResult.breed.conservation === 'common' ? 'bg-green-500/20 text-green-400 border-green-500/30' :
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <ul className="space-y-1 text-sm">
                      <li><span className="font-medium text-green-300">Colors:</span> <span className="text-muted-foreground">{(topResult.breed.characteristics?.color ?? []).join(', ')}</span></li>
                      <li><span className="font-medium text-green-300">Horn Size:</span> <span className="text-muted-foreground capitalize">{topResult.breed.characteristics?.hornSize ?? 'N/A'}</span></li>
                      <li><span className="font-medium text-green-300">Body Size:</span> <span className="text-muted-foreground capitalize">{topResult.breed.characteristics?.bodySize ?? 'N/A'}</span></li>
                    </ul>
                  </div>
                  <div>
